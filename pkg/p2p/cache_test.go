...
func testCacheGetOrRefillHelper(t *testing.T, config *CacheConfig) {
	...
	for i := 0; i < fileNum; i++ {
		...
		fileContent := []byte(getRandomString(1024 * 1024))
		for seg := range NewRangeSplit(0, 128*1024, int64(len(fileContent)), int64(len(fileContent))).AllParts() {
			wg.Add(1)
			go func(offset int64, size int) {
				...
				if isError == 2 {
					...
					if Assert.Equal(nil, err) {
						Assert.Equal(fileContent[offset:offset+int64(size)], res)
					} else {
						t.Log(err)
					}
				}
